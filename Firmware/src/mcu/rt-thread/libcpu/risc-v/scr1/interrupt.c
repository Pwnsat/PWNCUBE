...
static void m_ext_irq_handle(void)
{
    ...
    /* CubeSat: diagnostic + guard. Capture the active external IRQ, and NEVER
     * dispatch a spurious/out-of-range vector or a NULL handler. Previously an
     * external interrupt with no registered handler called isr_func == NULL,
     * faulted, and hung the core in handle_trap's while(1). */
    *(volatile unsigned int *)0xff6ff8c0 = 0xE47A0000U | (irq & 0xffffU);
    if (irq >= MAX_HANDLERS)
    {
        /* spurious / get_active() returned -1 (no line actually active) */
        *(volatile unsigned int *)0xff6ff8c8 = 0x5B090000U | (irq & 0xffffU);
    ...
    *(volatile unsigned int *)0xff6ff8c4 = (unsigned int)isr_func;
    ...
    if (isr_func != RT_NULL)
    {
        ...
        write_csr_ipic(IPIC_EOI, 0x0);
        rt_hw_interrupt_umask(irq);   /* re-enable only handled lines */
    }
    else
    {
        /* No handler registered: EOI and leave the line MASKED so it cannot
         * re-fire and crash the core. */
        *(volatile unsigned int *)0xff6ff8c8 = 0x4E554C00U | (irq & 0xffU);
        write_csr_ipic(IPIC_EOI, 0x0);
    }
...
void handle_trap(unsigned int i_mcause, unsigned int i_mepc, unsigned int i_sp)
{
    ...
    /* CubeSat: latch EVERY trap. Normal traffic is the machine timer
     * (0x80000007). If the core hangs, the LAST value here is the fatal cause. */
    *(volatile unsigned int *)0xff6ff8a0 = i_mcause;
    *(volatile unsigned int *)0xff6ff8a4 = i_mepc;
    ...
    {
        /* CubeSat: latch the FIRST fatal exception (write-once) with cause, PC,
         * SP and mtval (the faulting address for load/store faults) BEFORE any
         * rt_kprintf (console disabled -> could block) and before while(1). */
        static volatile int fatal_latched = 0;
        if (fatal_latched == 0)
        {
            fatal_latched = 1;
            *(volatile unsigned int *)0xff6ff8b0 = 0xFA7A0000U | (mcause & 0xffffU);
            *(volatile unsigned int *)0xff6ff8b4 = i_mepc;
            *(volatile unsigned int *)0xff6ff8b8 = i_sp;
            *(volatile unsigned int *)0xff6ff8bc = read_csr(0x343); /* mtval = fault addr */
        }
        /* system interrupt */
        switch (mcause)
        {
