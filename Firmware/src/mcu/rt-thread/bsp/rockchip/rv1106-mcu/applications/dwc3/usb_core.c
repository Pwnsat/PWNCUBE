...
int usb_init(void)
{
	...
	HDBG(0x16);
	*(volatile unsigned int *)0xff6ff9c8U = (unsigned)dev_index;   /* devices allocated at scan end */
	debug("scan end\n");
			      ...
			      struct usb_device *parent)
{
	 ...
	 */
	/* Trace where prepare fails for a FS device (the mouse behind the hub):
	 * stage in [19:16], err in [15:0]. 0xF = success. */
#define MRK_PREP(st, e) do { if (dev->speed == USB_SPEED_FULL) \
	*(volatile unsigned int *)0xff6ffa58U = \
		0x9A000000U | ((unsigned)(st) << 16) | ((unsigned)(e) & 0xffff); } while (0)
	...
	HDBG(0x62);
	if (err) { MRK_PREP(2, err); return err; }
	err = usb_hub_port_reset(dev, parent);
	HDBG(0x63);
	if (err) { MRK_PREP(3, err); return err; }
	...
	if (!do_read) {
		...
		if (err) {
			...
			if (dev->speed == USB_SPEED_FULL)
				*(volatile unsigned int *)0xff6ffa5cU =
					0x57A70000U | ((unsigned)dev->status & 0xffff);
			return err;
		}
	}
...
int usb_select_config(struct usb_device *dev)
{
	...
	*(volatile unsigned int*)0xff6ff9d4U = (unsigned)err;
	HDBG(0x82);
	if (err >= 0) {
	...
	/* Capture the hub's SET_CONFIGURATION outcome (idProduct 0x0101 = SL2.1A):
	 * cfgval, err, and dev->status. If the hub is left addressed-not-configured,
	 * it ACKs some requests but never actually powers its ganged VBUS switch. */
	if (dev->descriptor.idProduct == 0x0101)
		*(volatile unsigned int*)0xff6ffa4cU =
			0x5C000000U |
			((unsigned)(dev->config.desc.bConfigurationValue & 0xf) << 16) |
			((unsigned)(err & 0xff) << 8) |
			(unsigned)(dev->status & 0xff);
	HDBG(0x86);
	if (err < 0) {
		     ...
		     struct usb_device *parent)
{
	...
	HDBG(0x67);
	if (dev->speed == USB_SPEED_FULL)
		*(volatile unsigned int *)0xff6ffa5cU = 0x5C1F0000U | ((unsigned)ret & 0xffff);
