...
static int xhci_configure_endpoints(struct usb_device *udev, bool ctx_change)
{
	...
	if (!event)                     /* command timed out: no event to inspect */
		return -ETIMEDOUT;
...
static int xhci_address_device(struct usb_device *udev, int root_portnr)
{
	...
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
	if (!event)                     /* Address Device timed out */
		return -ETIMEDOUT;
	...
	/* FS diag: the assigned USB device address. Non-zero => the real SET_ADDRESS
	 * traversed the hub TT successfully (kills the "DEV_HUB/TT missing" theory). */
	if (udev->speed == USB_SPEED_FULL)
		*(volatile unsigned int *)0xff6ffa78U =
			0xADD00000u | (le32_to_cpu(slot_ctx->dev_state) & DEV_ADDR_MASK);
...
static int _xhci_alloc_device(struct usb_device *udev)
{
	...
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
	if (!event)                     /* Enable Slot timed out */
		return -ETIMEDOUT;
...
/*
 * Non-DM: tell the xHCI controller this device is a HUB. The DM stack does this
 * from usb_hub_configure() via usb_update_hub_device() -> xhci_update_hub_device(),
 * but that whole path is compiled out for the legacy (non-DM) stack. Without the
 * DEV_HUB bit / port count / TT think time in the hub's slot context, the
 * controller does not treat the device as a hub and mishandles everything routed
 * downstream of it (Address Device of child devices, split transactions). ttt is
 * the TT Think Time field (0..3) taken from the hub's wHubCharacteristics[6:5].
 * Single-TT is assumed, which is correct for the SL2.1A and typical HS hubs.
 */
int xhci_update_hub_device_nodm(struct usb_device *udev, unsigned int ttt)
{
	struct xhci_ctrl *ctrl = xhci_get_ctrl(udev);
	struct xhci_virt_device *virt_dev;
	struct xhci_input_control_ctx *ctrl_ctx;
	...
	/* Ignore the emulated root hub: it has no real device slot. */
	if (!udev->parent)
		return 0;
	...
	/* Match Linux's xhci_update_hub_device() exactly: a SLOT_FLAG-only Configure
	 * Endpoint that copies the slot context and sets the hub fields. The xHC
	 * consumes DEV_HUB / Number-of-Ports / TT-Think-Time internally and is NOT
	 * required to mirror them into the output device context, so reading them back
	 * proves nothing — rely on the command completion code only. */
	ctrl_ctx = xhci_get_input_control_ctx(virt_dev->in_ctx);
	ctrl_ctx->add_flags = cpu_to_le32(SLOT_FLAG);
	ctrl_ctx->drop_flags = 0;
	...
	slot_ctx->dev_info  &= cpu_to_le32(~DEV_MTT);          /* single-TT hub */
	slot_ctx->dev_info2 |= cpu_to_le32(XHCI_MAX_PORTS(udev->maxchild));
	slot_ctx->tt_info   |= cpu_to_le32(TT_THINK_TIME(ttt));

	/* Trustworthy oracle (agent A): dump the INPUT context the xHC actually reads
	 * for this hub Configure Endpoint — output-ctx readback is spec-exempt for
	 * Hub/TTT (xHCI 6.2.2). If these are malformed (SLOT_FLAG clear, DEV_HUB clear,
	 * wrong slot_id, or CSZ=1 while the driver strides 32) that IS the bug. */
	{
		u32 hcc = xhci_readl(&ctrl->hccr->cr_hccparams);
		*(volatile unsigned int *)0xff6ff9a4U = le32_to_cpu(ctrl_ctx->add_flags);
		*(volatile unsigned int *)0xff6ff9a8U = le32_to_cpu(slot_ctx->dev_info);
		*(volatile unsigned int *)0xff6ff9acU = le32_to_cpu(slot_ctx->dev_info2);
		*(volatile unsigned int *)0xff6ff9b0U = le32_to_cpu(slot_ctx->tt_info);
		*(volatile unsigned int *)0xff6ff9b4U = ((unsigned)slot_id << 4) | ((hcc >> 2) & 1u);
	}
