...
/*
 * USB 2.0 spec 11.24.2.3 Clear_TT_Buffer. When a split transaction to a FS/LS
 * device behind a high-speed hub errors/STALLs, the hub's Transaction Translator
 * keeps the failed transaction buffered and NAKs/errors every subsequent split
 * to that endpoint — so a host-side endpoint reset alone is not enough and the
 * retry deterministically re-fails. This tells the parent hub to flush that TT
 * buffer. For a control endpoint both directions must be cleared. Called from
 * the control-transfer stall-recovery path in xhci_ring.c.
 */
int usb_hub_clear_tt(struct usb_device *dev, int ep_num, int is_control, int dir_in)
{
	struct usb_device *hub = dev->parent;
	unsigned int devinfo;
	int tt;

	/* Only FS/LS devices behind a real high-speed hub have a TT to clear. */
	if (!hub || hub->speed != USB_SPEED_HIGH)
		return 0;
	if (dev->speed != USB_SPEED_FULL && dev->speed != USB_SPEED_LOW)
		return 0;

	devinfo  = (unsigned)(ep_num & 0xf);
	devinfo |= ((unsigned)(dev->devnum & 0x7f)) << 4;
	devinfo |= is_control ? 0u : (2u << 11);   /* EP type: 00=control, 10=bulk */
	tt = dev->portnr;                          /* single-TT: the device's hub port */

	if (is_control) {
		/* A control endpoint moves data both ways — clear IN and OUT. */
		usb_control_msg(hub, usb_sndctrlpipe(hub, 0), HUB_CLEAR_TT_BUFFER,
				USB_RT_PORT, devinfo | 0x8000, tt, NULL, 0, USB_CNTL_TIMEOUT);
		usb_control_msg(hub, usb_sndctrlpipe(hub, 0), HUB_CLEAR_TT_BUFFER,
				USB_RT_PORT, devinfo, tt, NULL, 0, USB_CNTL_TIMEOUT);
	} else {
		if (dir_in)
			devinfo |= 0x8000;
		usb_control_msg(hub, usb_sndctrlpipe(hub, 0), HUB_CLEAR_TT_BUFFER,
...
static void usb_hub_power_on(struct usb_hub_device *hub)
{
	...
	{
		/* Instrument the port-power path for the downstream hub (devnum>=2):
		 * maxchild + bPwrOn2PwrGood, and the OR of every SetPortFeature return. */
		unsigned acc = 0;
		int dbg = (dev->devnum >= 2);
		if (dbg) {
			*(volatile unsigned int*)0xff6ffa40U =
				((unsigned)(dev->devnum & 0xff) << 24) |
				((unsigned)(dev->maxchild & 0xff) << 16) |
				(unsigned)(hub->desc.bPwrOn2PwrGood & 0xffff);
			/* wHubCharacteristics: bits[1:0] power-switch mode
			 * (00 ganged, 01 per-port, 1x none) + bit[7:5] flags. */
			*(volatile unsigned int*)0xff6ffa48U =
				0xC0DE0000U | (unsigned)(hub->desc.wHubCharacteristics & 0xffff);
		}
		for (i = 0; i < dev->maxchild; i++) {
			int r = usb_set_port_feature(dev, i + 1, USB_PORT_FEAT_POWER);
			acc |= ((unsigned)r & 0xf) << (i * 4);
			debug("port %d returns %lX\n", i + 1, dev->status);
		}
		if (dbg)
			*(volatile unsigned int*)0xff6ffa44U = 0x5E700000U | acc;
	}
	 ...
	 */
	hub->connect_timeout = hub->query_delay + 60000;  /* ponytail: wide window so a hand-timed re-plug lands inside the scan */
...
static inline char *portspeed(int portstatus)
{
	...
	*(volatile unsigned int*)0xff6ff9dcU = portstatus;
	HDBG(0x90);
...
int usb_hub_port_connect_change(struct usb_device *dev, int port)
{
	...
	if (((!(portstatus & USB_PORT_STAT_CONNECTION)) &&
		...
		if (!(portstatus & USB_PORT_STAT_CONNECTION))
			return -ENOTCONN;
		/* Still connected AND this port already has an enumerated child:
		 * this is a duplicate connect-change — a contact bounce during the
		 * scan window re-asserts C_CONNECTION for a device we already brought
		 * up. Re-running enumeration would allocate a second device slot for
		 * the same physical device and inflate the device count, so treat the
		 * re-report as a no-op. */
...
retry:
	 ...
	 * resetting too soon leaves a FS/LS device behind a HS hub's TT in an
	 * unstable state, which makes its first GET_DESCRIPTOR split stall flakily. */
	mdelay(100);
	...
	/* Reset recovery (USB 2.0 §7.1.7.5, TRSTRCY >= 10 ms): the device must be
	 * given time after reset before the first control transfer (SET_ADDRESS).
	 * Use a generous value for the FS-through-TT path. */
	mdelay(50);
	...
	*(volatile unsigned int*)0xff6ff9dcU = portstatus;
	...
	/* Enumeration recurses (root hub -> SL2.1A -> reader), so a single-value
	 * marker is overwritten by the OUTER connect_change that writes last. Gate
	 * the reader diagnostics on the parent hub being a downstream hub
	 * (devnum >= 2; the emulated root hub is devnum 1) so we see the reader's
	 * own path, not the SL2.1A's. */
	int dbg = (dev->devnum >= 2);
	/* The FS device's first GET_DESCRIPTOR split through the hub TT is marginal:
	 * it stalls flakily (proven — the same enumeration succeeds on some attempts,
	 * stalls on others). An EP0-level retry re-hits the same stall, but a fresh
	 * port reset re-inits the device (address back to 0) and the hub TT state, and
	 * the split usually completes within a few attempts. Linux retries enumeration
	 * the same way (hub_port_init / PORT_INIT_TRIES). */
	int enum_tries = 6;

enum_retry:
	if (dbg) *(volatile unsigned int*)0xff6ff9ecU = 0xA0000000U | (((port + 1) & 0xff) << 4) | (speed & 0xf);
	ret = usb_alloc_new_device(dev->controller, &usb);
	if (dbg) *(volatile unsigned int*)0xff6ff9f0U = 0xB0000000U | ((unsigned)ret & 0xffff);
	if (ret) {
	...
	if (dbg) {
		*(volatile unsigned int*)0xff6ff9e0U = (unsigned)ret;
		*(volatile unsigned int*)0xff6ff9ccU = 0xD0000000U | ((unsigned)usb->devnum & 0xffff)
		                                     | ((unsigned)(usb->descriptor.idVendor ? 1 : 0) << 16);
	}
	HDBG(0x91);
	if (ret < 0) {
		/* Woops, disable the port */
		if (dbg) *(volatile unsigned int*)0xff6ff9f4U = 0xC0000000U | ((unsigned)ret & 0xffff);
		usb_free_device(dev->controller);
		dev->children[port] = NULL;
		if (--enum_tries > 0) {
			/* fresh port reset re-inits device + hub TT, then retry enumeration */
			if (dbg) *(volatile unsigned int*)0xff6ff9f8U = 0x71E50000U | (enum_tries & 0xff);
			if (usb_hub_port_reset(dev, port, &portstatus) >= 0)
				goto enum_retry;
		}
	} else {
		if (dbg) *(volatile unsigned int*)0xff6ff9f4U = 0xCEED0000U;
	}
#endif
	if (ret < 0) {
...
static int usb_scan_port(struct usb_device_scan *usb_scan)
{
	...
	portstatus = le16_to_cpu(portsts->wPortStatus);
	portchange = le16_to_cpu(portsts->wPortChange);
	*(volatile unsigned int*)0xff6ff9d8U = (portstatus | (portchange<<16));
	debug("Port %d Status %X Change %X\n", i + 1, portstatus, portchange);

	/* Scan ring: log EVERY (hub, port) status seen, unmasked by recursion.
	 * Reset at the first port of the root hub (devnum 1). 16 slots at
	 * 0xff6ffa00; entry = devnum<<24 | port<<16 | portstatus. */
	{
		static unsigned s_scan_n;
		if (dev->devnum <= 1 && i == 0)
			s_scan_n = 0;
		*(volatile unsigned int *)(0xff6ffa00U + ((s_scan_n & 0xf) << 2)) =
			((unsigned)(dev->devnum & 0xff) << 24) |
			((unsigned)((i + 1) & 0xff) << 16) | portstatus;
		*(volatile unsigned int *)0xff6ff9c4U = ++s_scan_n;   /* total scans */
	}
...
static int usb_device_list_scan(void)
{
	...
	while (1) {
		...
		list_for_each_entry_safe(usb_scan, tmp, &usb_scan_list, list) {
			...
			/* Scan this port */
			*(volatile unsigned int*)0xff6ff9d0U += 1;
...
/* Non-DM hub-context update (implemented in xhci.c): marks the slot as DEV_HUB. */
extern int xhci_update_hub_device_nodm(struct usb_device *udev, unsigned int ttt);
...
#if !CONFIG_IS_ENABLED(DM_USB)
	/*
	 * Non-DM equivalent of the DM usb_update_hub_device() below: tell the xHCI
	 * controller this is a hub (DEV_HUB + port count + TT think time). Without
	 * it the legacy stack never marks the slot as a hub and the controller
	 * mishandles all downstream traffic. TT Think Time = wHubCharacteristics[6:5].
	 */
	{
		int hret = xhci_update_hub_device_nodm(dev,
						(hubCharacteristics >> 5) & 0x3);
		*(volatile unsigned int *)0xff6ffa54U = 0xDE00B000U | ((unsigned)hret & 0xfff);
		if (hret < 0 && hret != -ENOSYS)
			debug("%s: non-DM hub ctx update failed (%d)\n", __func__, hret);
	}
#endif
