...
{
	/* ponytail: RV1106 host — Linux's dwc3_core_soft_reset EARLY-RETURNS for
	 * dr_mode=host and never toggles GUSB2PHYCFG.PHYSOFTRST. Asserting the USB2
	 * PHY soft-reset via the controller here disrupts the innosilicon PHY and
	 * leaves the root port stuck in Polling / LS (HS chirp never detected). So we
	 * skip the USB2 PHY reset entirely; the innosilicon PHY is already brought up
	 * (clocks/POR/tuning/un-suspend) on the probe.c side. The USB3 pipe reset is
	 * harmless (no SS PHY) but also unnecessary, so we skip it too. */
	(void)dwc3_reg;
}
