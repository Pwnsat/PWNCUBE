...
union xhci_trb *xhci_wait_for_event(struct xhci_ctrl *ctrl, trb_type expected)
{
	...
	HDBG(0x30);
	*(volatile unsigned int *)0xff6ff9c4U = 0x3000U | (unsigned)expected;

	do {
		union xhci_trb *event = ctrl->event_ring->dequeue;

		if ((++iter & 0xffffU) == 0)
			*(volatile unsigned int *)0xff6ff9ccU = iter;   /* spin heartbeat */
...
/*
 * Recover a control endpoint the xHC halted on a STALL. Unlike abort_td (which
 * Stops a running ring), a halted EP needs a Reset Endpoint command followed by
 * Set TR Dequeue to move past the failed TD, before it can be reused. FS/LS
 * devices behind a HS hub's TT routinely STALL the first control-IN and succeed
 * on a clean retry — without this the endpoint stays halted forever.
 */
static void reset_ep_halt(struct usb_device *udev, int ep_index)
{
	struct xhci_ctrl *ctrl = xhci_get_ctrl(udev);
	struct xhci_ring *ring = ctrl->devs[udev->slot_id]->eps[ep_index].ring;
	union xhci_trb *event;
			...
			void *buffer)
{
	...
	int tries = 2;   /* one retry after an ep0 halt (STALL) recovery */
 ...
 retry:
	...
	/* Diag (FS): the ep0 OUTPUT context state going INTO this control transfer.
	 * ep_info[2:0]=EP State (1=Running,2=Halted,3=Stopped,4=Error), ep_info2 has
	 * EP-type[5:3] + max-packet[31:16]. And the slot dev_state (addr + slot state)
	 * to compare against Linux at the same point. */
	if (udev->speed == USB_SPEED_FULL) {
		struct xhci_slot_ctx *sc = xhci_get_slot_ctx(ctrl, virt_dev->out_ctx);
		*(volatile unsigned int *)0xff6ffa88U = le32_to_cpu(ep_ctx->ep_info);
		*(volatile unsigned int *)0xff6ffa8cU = le32_to_cpu(ep_ctx->ep_info2);
		*(volatile unsigned int *)0xff6ffa90U = le32_to_cpu(sc->dev_state);
	}
	...
	record_transfer_result(udev, event, length);
	/* Debug (FS only): completion code + which TRB (stage) the event points at,
	 * so we can see whether SETUP/DATA/STATUS of the split control transfer fails.
	 * stage = (event_trb - start_trb) / 16 : 0=SETUP,1=DATA,2=STATUS. */
	if (udev->speed == USB_SPEED_FULL) {
		unsigned long long evtrb = le64_to_cpu(event->trans_event.buffer);
		unsigned cc = GET_COMP_CODE(le32_to_cpu(event->trans_event.transfer_len));
		unsigned stage = (unsigned)(((uintptr_t)evtrb - (uintptr_t)start_trb) / 16u) & 0xff;
		*(volatile unsigned int *)0xff6ffa74U = 0xCC000000u | (cc << 8) | stage;
		/* raw pointers to verify identity mapping / real stage (evtrb is a bus
		 * addr, start_trb a VA — if they map 1:1 the stage math is valid). */
		*(volatile unsigned int *)0xff6ffa7cU = (unsigned)(uintptr_t)evtrb;
		*(volatile unsigned int *)0xff6ffa80U = (unsigned)(uintptr_t)start_trb;
	}
	xhci_acknowledge_event(ctrl);

	/* On a STALL the xHC halted EP0. Recover (Reset Endpoint + Set TR Dequeue)
	 * and retry the whole control transfer once. If the retry succeeds it was a
	 * transient TT/timing stall; if it stalls again it is persistent (device not
	 * really reachable at this address). Marker records that a retry was taken. */
	if (udev->status == USB_ST_STALLED && --tries > 0) {
		extern int usb_hub_clear_tt(struct usb_device *dev, int ep_num,
					    int is_control, int dir_in);
		*(volatile unsigned int *)0xff6ffa84U = 0x51A11000u;   /* pre-retry: stalled */
		reset_ep_halt(udev, ep_index);       /* host side: un-halt EP0        */
		usb_hub_clear_tt(udev, 0, 1, 0);     /* hub side: flush the stuck TT buffer */
