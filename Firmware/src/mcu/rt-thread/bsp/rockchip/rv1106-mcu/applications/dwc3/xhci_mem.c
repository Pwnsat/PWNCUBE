				     ...
				     struct usb_device *udev, int hop_portnr)
{
...
#else
	/* Non-DM: build the xHCI route string from the legacy parent chain. Each
	 * hub between the device and the root hub contributes one nibble; the hub
	 * closest to the root goes in the lowest nibble. A device directly on a
	 * root port keeps route 0. Without this the controller addresses a device
	 * behind an external hub as if it were on the root port, so nothing behind
	 * the hub ever enumerates (Address Device / control transfers mis-route). */
	{
		struct usb_device *d;
		for (d = udev; d->parent && d->parent->parent; d = d->parent) {
			unsigned p = (d->portnr > 15) ? 15 : (unsigned)d->portnr;
			route = (route << 4) | (p & 0xf);
		}
	}
	debug("route string %x\n", route);
...
#else
	/* Non-DM: a FS/LS device behind a HS hub needs the split-transaction TT
	 * fields set. Walk up to the nearest HS ancestor hub: TT_SLOT is that
	 * hub's slot, TT_PORT is the port on it that leads to the FS/LS device.
	 * Single-TT assumed (no DEV_MTT) — correct for the SL2.1A and most hubs.
	 * ponytail: single-TT only; add multi-TT if a DEV_MTT hub shows up. */
	if (speed == USB_SPEED_LOW || speed == USB_SPEED_FULL) {
		struct usb_device *d = udev;
		while (d->parent && d->parent->parent &&
		       d->parent->speed != USB_SPEED_HIGH)
			d = d->parent;
		if (d->parent && d->parent->speed == USB_SPEED_HIGH) {
			slot_ctx->tt_info |= cpu_to_le32(TT_PORT(d->portnr));
			slot_ctx->tt_info |= cpu_to_le32(TT_SLOT(d->parent->slot_id));
			/* Dump the mouse's TT wiring: parent(HS hub) slot + its port. */
			*(volatile unsigned int *)0xff6ffa68U =
				0x77000000U | ((unsigned)(d->parent->slot_id & 0xff) << 8) |
				(unsigned)(d->portnr & 0xff);
		}
	}
#endif
	...
	case USB_SPEED_SUPER:
		...
		break;
	case USB_SPEED_HIGH:
	/* USB core guesses at a 64-byte max packet first for FS devices; it is
	 * corrected via Evaluate Context after the 8-byte descriptor header is read
	 * (xhci_check_maxpacket). Matches Linux and the live golden capture, which
	 * enumerates this exact FS mouse through this hub with ep0 mp = 64. */
	case USB_SPEED_FULL:
	...
	/* Dump the FS device's final slot context so we can verify route string,
	 * speed and TT wiring against what the controller needs for splits. */
	if (speed == USB_SPEED_FULL) {
		*(volatile unsigned int *)0xff6ffa60U = le32_to_cpu(slot_ctx->dev_info);
		*(volatile unsigned int *)0xff6ffa64U = le32_to_cpu(slot_ctx->tt_info);
	}
